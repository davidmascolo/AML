...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        jmodel = np.mean(-np.log(probs[range(N),y]))
        
        w1norm = np.linalg.norm(W1)**2
        w2norm = np.linalg.norm(W2)**2
        
        loss = jmodel + reg * (w1norm + w2norm)
        ...
        delta = np.zeros([N, W2.shape[1]])
        for i in range(N):
            delta[i, y[i]] = 1
        
        
        
        diff = 1/N * (scores - delta)
        grads["W2"] = a2.T.dot(diff) + 2*reg*W2
        
        cond = np.where(z2 == a2, 1, 0)
        der_z2 = cond * diff.dot(W2.T)
        
        grads["W1"] = np.dot(a1.T, der_z2) + 2*reg*W1
        
        grads["b1"] = np.sum(der_z2, axis = 0)
        
        grads["b2"] = np.sum(diff, axis = 0)
    ...
    def predict(self, X):
        ...
        a1 = X
        z2 = (np.dot(a1, self.params['W1'])) + self.params['b1'] 
        a2 = np.maximum(0,z2)       
