...
## 2 layers, reg = 0.001, batch_size = 100, num_epochs = 30
## acc = 53.8% on validation and 53.0% on test

## 3 layers, reg = 0.001, batch_size = 100, num_epochs = 30
## acc = 53.4% on validation and 53.8% on test
...
if train:
    ...
    model.train() #set dropout and batch normalization layers to training model
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            for images, labels in val_loader:
                ...
                images = images.view(images.size(0), -1)
                outputs_data = model(images)
                ## the class with the highest energy is what we choose as prediction
                predicted = torch.argmax(outputs_data, dim = 1)
